class DataShape {
    constructor(typeOfShape, data, scrollArea, move) {
        ...
        this._opacity = Math.floor(math.random(2, 10)) / 10;
        if (typeOfShape === 1){
            this._speed = 0.09;
    ...
    createShape (){
        this.$shape.css({top:this._top + 'px', left:this._left + 'px', width:this._width + 'px', height:this._height + 'px', opacity: this._opacity}).addClass('dataShape');
...
$scrollArea.scroll(function(e){
    ...
    if ($('.orangeBox').visible(true)) {
        $('#yoram').addClass('inView');
    } else {
        $('#yoram').removeClass('inView');
    }
});
